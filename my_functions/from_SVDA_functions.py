...
def ATL03_signal_photons(fname, ATL03_output_path, ROI_fname, EPSG_Code, reprocess=False):
    ...
    for b in gtr:
        ...
        # Along-track distance in a segment projected to the ellipsoid of the received photon, based on the Along-Track Segment 
        # algorithm. Total along track distance can be found by adding this value to the sum of segment lengths measured from
        # the start of the most recent reference groundtrack
        ...
        # calculate alongtrack distance that starts with 0 at the first photon (final distance should be ~60000 m)
...
def ATL08_minimal(fname, ATL08_output_path, ROI_fname, EPSG_Code, reprocess=False):

    # read h5 file in (closed h5 format)
    ATL08 = h5py.File(fname,'r')
    ...
    for b in gtr:
        print('Opening %s: %s' % (os.path.basename(fname), b))
        if reprocess==False and os.path.exists(os.path.join(ATL08_output_path,'ATL08_Land_%s_%s.hdf'%('_'.join(os.path.basename(fname).split('_')[1:2]),b))):
            #print('File %s already exists and reprocessing is turned off.\n' % os.path.join(ATL08_output_path,'ATL08_Land_%s_%s.hdf'%('_'.join(os.path.basename(fname).split('_')[1:2]),b)))
            print('File with %s already exists and reprocessing is turned off.\n' % b)
            continue
        attribute_lat = b + '/land_segments/latitude'
        lat = np.asarray(ATL08[attribute_lat]).tolist()
        attribute_lon = b + '/land_segments/longitude'
        lon = np.asarray(ATL08[attribute_lon]).tolist()
        attribute_h = b + '/land_segments/terrain/h_te_best_fit'
        h = np.asarray(ATL08[attribute_h]).tolist()

        # set up dataframe
        ATL08_df = pd.DataFrame({'Latitude': lat, 'Longitude': lon, 
                                'best_fit_Height': h})
        ...
        # delete variables not in use anymore
        del lat, lon, h
        del attribute_lat, attribute_lon, attribute_h
        ...
        ATL08_df['alongtrack'] = ATL08_df['alongtrack'] + 90
        ...
        # calculate alongtrack distance that starts with 0 at the first photon (final distance should be ~60000 m)
        ATL08_df['alongtrack_base'] = ATL08_df['alongtrack'] - ATL08_df['alongtrack'].iloc[0] * np.ones(len(ATL08_df))

        # if output dir doesnt exist, create it
        if not os.path.exists(ATL08_output_path):
            os.mkdir(ATL08_output_path)

        # write ATL08 df to hdf
        ATL08_df.to_hdf(os.path.join(ATL08_output_path,'ATL08_Land_%s_%s.hdf'%('_'.join(os.path.basename(fname).split('_')[1:2]),b)),
                        key='ATL08_%s_%s'%('_'.join(os.path.basename(fname).split('_')[1::])[:-4],b), complevel=7)
        print('saved to %s'%os.path.join(ATL08_output_path,'ATL08_Land_%s_%s.hdf'%('_'.join(os.path.basename(fname).split('_')[1:2]),b)))
        print()
    ATL08.close()